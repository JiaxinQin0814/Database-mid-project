...
window.onload = function(){
	var oBox = document.getElementById('index-ztbox');
	var oConter = document.getElementById('timesheet-default');
	var oUl = oConter.getElementsByTagName('ul')[0];
	var oscale=oConter.getElementsByTagName('div')[0];
	//var oLi = oConter.getElementsByTagName('li');
	var oScroll = document.getElementById('index-scroll');
	var oSpan = oScroll.getElementsByTagName('span')[0];
	var i = 0;
	oUl.style.width = 1200 +'px';
	oscale.style.width = 1200 +'px';
	...
	function oscroll(l){
		...
		sMove(oscale, 'left', '-'+Math.ceil((oscale.offsetWidth - (oConter.offsetWidth + 15)) * scrol));
	}
	...
	function td(l){
		...
		oscale.style.left = '-'+(oscale.offsetWidth - (oConter.offsetWidth + 15)) * scrol +'px';
	}
};
//运动框架
