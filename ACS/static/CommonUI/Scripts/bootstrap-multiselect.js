...
!function($) {
    ...
    Multiselect.prototype = {
        ...
        defaults: {
            ...
            buttonText: function(options, select) {
                if (options.length === 0) {
                    ...
                    return this.nonSelectedText;//update by cxy
                }
                else {
                    if (options.length > this.numberDisplayed) {
                        //return options.length + ' ' + this.nSelectedText + ' <b class="caret"></b>';
                        return options.length + ' ' + this.nSelectedText ;//update by cxy
                    }
                    else {
                        var selected = '';
        ...
        templates: {
            button: '<button type="button" class="multiselect dropdown-toggle form-control t-select1"  data-toggle="dropdown"></button>',//update by cxy
        ...
        buildDropdownOptions: function() {
            ...
            this.$select.children().each($.proxy(function(index, element) {
                var sID = this.$select.attr("id");
                if (sID != undefined && sID != null && sID != "")
                {
                    $(element).attr("name", sID);
                    $(element).attr("validate", this.$select.attr("validate"));
                }
        ...
        createOptionValue: function(element) {
            ...
            $checkbox.val(value);// id="' + $.trim(value) + '
            $checkbox.attr("id", value);
            this.$container.attr("id", $(element).attr("name") + "div")
            //this.$container.attr("validate", $(element).attr("validate"))
            $(this.templates.button).attr("id", $(element).attr("name") + "bt");
            $(this.templates.button).attr("validate", $(element).attr("validate"));
            $checkbox.attr("name", $(element).attr("name")+"M");
            $checkbox.attr("class", "error");
