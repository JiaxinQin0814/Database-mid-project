...
(function ($) {
    $.fn.extend({
        'Airman_EditedSelect': function (li, callback) {
            ...
            h += '</div>'
            h += '<div id="Plug_in_Div_2" class="t-select-in2 display-hide">'
            h += '<ul>'
            h += li;
            h += '</ul>'
            h += '</div>'
        ...
        'Airman_EditedSelect_GetText': function () {
            return $('#Plug_in_Text').val()
        },
        'Airman_EditedSelect_GetValue': function () {
            return $('#Plug_in_Text').attr('data-value')
        },
        'Airman_EditedSelect_SetText': function (s) {
            $('#Plug_in_Text').val(s)
        },
