...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderSlatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			classStartNum=classStartNum+1;
			slotDate = this.start.clone().time(slotTime);
			isLabeled = isInt(divideDurationByDuration(slotTime, this.labelInterval));

			this.crew_classTime_Arr=view.opt('crew_classTime_Arr'); //add by cxy (xiugai ¿Î½Ú)

			if(this.crew_classTime_Arr!=undefined && this.crew_classTime_Arr!=""){

				//alert(this.crew_classTime_Arr.get(0).start);
			axisHtml =
				'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + '>' +
					(isLabeled ?
						'<span >' + // for matchCellWidths
							htmlEscape(slotDate.format(this.labelFormat))+'</span><br/><span>(' + this.crew_classTime_Arr[classStartNum-1] +
						')</span>' :
						''
						) +
				'</td>';
				...
				axisHtml =
					'<td class="fc-axis fc-time ' + view.widgetContentClass + '" ' + view.axisStyleAttr() + ' style="text-align: center;">' +
					(isLabeled ?
						'<span >' + // for matchCellWidths
