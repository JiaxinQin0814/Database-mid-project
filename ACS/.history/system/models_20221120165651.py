...
from django.core.validators import MaxValueValidator,MinValueValidatorBaseUserManager
...
class MyUser(AbstractBaseUser,PermissionsMixin):
    ...
    Id = models.AutoField(unique=True,verbose_name="职工编号",primary_key=True)
    email = models.EmailField(verbose_name="邮箱")
    school =models.ForeignKey(School,null=True,blank=True,on_delete=models.CASCADE,related_name="teacher_school",verbose_name="职工所属学院")
    attribute = (
        ("teacher","教师"),
        ("admin","教秘")
    )
    name = models.CharField(max_length=16, blank=False, verbose_name="职工姓名")
    kind = models.CharField(max_length=10,choices=attribute,default="教师",verbose_name="用户类别")
    
    USERNAME_FIELD = 'Id' # 使用authenticate验证时使用的验证字段，可以换成其他字段，但验证字段必须是唯一的，即设置了unique=True
